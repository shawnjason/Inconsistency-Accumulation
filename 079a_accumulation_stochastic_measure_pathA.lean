/-
# ID 079a: Measure-Theoretic Stochastic Accumulation — Path A

Catalog ID 79 (Inconsistency Accumulation paper, Theorem 1 stochastic clause,
full measure-theoretic form, Path A: fiber-partitioning route).
Companion: ID 079b in 079b_accumulation_stochastic_measure_pathB.lean
(Path B: product-isomorphism route — independent proof of the same
load-bearing product-measure independence identity).
Companion: ID 062b in 062b_accumulation_stochastic.lean (finite-
combinatorial form of the same theorem).
Companion: ID 062a in 062a_accumulation_deterministic.lean (deterministic
clause of Theorem 1).

This file is the measure-theoretic formalization of Theorem 1's stochastic
clause in the paper
...
The load-bearing step — the product-measure independence identity
underlying §5's tower-property argument — is closed via fiber-
partitioning with an explicit coordinate-update bijection, using
`Finset.sum_fiberwise`, `Function.update`, and `Finset.sum_bij`.
Path B (in the companion file) closes the same identity through
the canonical product isomorphism via `Fin.insertNthEquiv` and
`Fintype.sum_prod_type`. Either file compiles in isolation; neither
depends on the other. The shared theorem is therefore certified
through two independent machine-checked proofs.
...
- The bridge lemma (section 4) is the load-bearing step; every other
  step is bookkeeping around it
- This file is logically downstream of `accumulation_stochastic.lean`:
  the combinatorial file establishes the per-window failure probability
  bound that this file integrates against the trajectory measure.
