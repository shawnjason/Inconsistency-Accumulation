...
-- ID 062b: Linear Inconsistency Accumulation, Stochastic Clause (Theorem 1)
--
-- Catalog ID 62 (Inconsistency Accumulation paper, Theorem 1 stochastic clause,
-- finite-combinatorial form).
-- Companion: ID 062a in accumulation_deterministic.lean (deterministic clause).
...
-- is the sum of per-block failure probabilities. The full measure-
-- theoretic form (with trajectory probability space, filtration, and
-- conditional-expectation tower-property argument) is proved
-- independently in the two measure-theoretic path files.
--
-- Shawn Kevin Jason
...
/-- Probability that policy `π` commits a non-admissible action in
    block `B`. The probability is the sum of policy mass placed on
    actions that fall outside the block's admissibility predicate. -/
...
/-- Pigeonhole on probabilities: in a finite non-empty action space,
    ...
    the total would fall short of 1. This is the load-bearing
    combinatorial fact that converts the policy-independent floor
    into the per-window failure-probability bound. -/
...
/-- Theorem 1, stochastic clause (catalog ID 62 / IA Theorem 1
    stochastic case, finite-combinatorial form).
    ...
    Proof structure: (1) pigeonhole-on-probabilities yields an action
    u_max with policy mass ≥ 1/|U|; (2) since |U| ≥ 2, an alternative
    g ≠ u_max exists; (3) construct an adversarial block whose
    admissibility predicate excludes u_max, replicate N times; (4)
    compute the expected inconsistency by linearity of expectation
    and pigeonhole-bounded per-block failure probability. -/
